...
fn is_subtype(sub: &Type, sup: &Type) -> bool {
    match (sub, sup) {
        ...
        (Type::Variant(sub_cases), Type::Variant(sup_cases)) => sub_cases == sup_cases,
        (_, _) => false
    }
}
...
fn check_if(g: Result<Type,String>, t: Result<Type,String>, e: Result<Type,String>)
            -> Result<Type,String> {
    match (g, t, e) {
        (Ok(Bool), Ok(u), Ok(v)) if is_subtype(&u, &v) => Ok(v),
        (Ok(Bool), Ok(u), Ok(v)) if is_subtype(&v, &u) => Ok(u),
        (Ok(Bool), Ok(u), Ok(v)) =>
            Err(format!("If branch types {:?} and {:?} are not compatible [T-IF]", u, v)),
